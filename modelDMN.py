...
class InputModule(nn.Module):
    ...
    Question Answering '''
    def positional_encoder(self, embedded_sentence):
        ...
        mat = embedded_sentence*Variable(l)
    ...
    def forward(self, input, word_embedding):
        ...
        input = input.view(input_shape[0], input_shape[1], input_shape[2], -1)
        input = self.positional_encoder(input)
        input = self.dropout(input)

        h0 = Variable(torch.zeros(2, input.size()[0], self.hidden_size)) # Initializing the initial hidden state (at t=0 time step)
        h0 = h0.cuda()
...
class AttnGRUCell(nn.Module):
    ...
    def forward(self, fact, hi_1, g):
        ...
        h_tilda = F.tanh(self.W(fact) + r_i*self.U(hi_1))
        g = g.unsqueeze(1)
...
class MemoryModule(nn.Module): # Takes facts, question and prev_mem as its and output next_mem
    ...
    def gateMatrix(self, facts, questions, prev_mem):
        ...
        # z.size() = (batch_size, num_sentences, 4*embedding_length)
        batch_size, _, embedding_length = questions.size()
...
class DMN(nn.Module):
    def __init__(self, hidden_size, vocab_size, num_pass=3):
        super(DMN,self).__init__()
        self.num_pass = num_pass
        self.word_embedding = nn.Embedding(vocab_size, hidden_size, padding_idx=0, sparse=False)
    ...
    def loss(self,context, questions, targets):
        ...
        correct = (pred_id.data == targets.data)
